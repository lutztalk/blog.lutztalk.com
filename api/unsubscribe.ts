...
export default async function handler(
  ...
  try {
    ...
    // Remove from Resend Audience (mark as unsubscribed)
    const resend = getResend();
    const audienceId = process.env.RESEND_AUDIENCE_ID?.trim();
    
    if (resend && removed > 0) {
      try {
        const updateData: { email: string; unsubscribed: boolean; audienceId?: string } = {
        ...
        console.log(`[Unsubscribe] Marked ${normalizedEmail} as unsubscribed in Resend${audienceId ? ` Audience ${audienceId}` : ' (default audience)'}`);
      } catch (err) {
